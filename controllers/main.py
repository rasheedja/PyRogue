...
class GameController:
    ...
    def __init__(self, width, height, fps,
        ...
        self.input_controller = \
            controllers.input.InputController(self.player_user)

    def main(self):
        """The main loop, runs while the game is open."""
        while not self.main_view.tdl.event.is_window_closed():
            for players in self.players:
                self.main_view.draw_char(players.x, players.y, players.char,
                                         players.fg_colour, players.bg_colour)

            self.main_view.update()

            for players in self.players:
                self.main_view.clear_char(players.x, players.y)
